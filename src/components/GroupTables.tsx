...
import { cancelGroupRequest, leaveGroup } from "../service/UserService/UserService."
...
export default function GroupTables({groups, waitingApproval, search}:{groups:GroupType[], waitingApproval:PendingGroupType[], search:String }){
    ...
    function cancelRequest(groupCode:string){

        async function cancel(){
            let l = await cancelGroupRequest(groupCode);
        ...
        confirmationState.getState().setConfirmationState({
            msg:"Do you want to cancel request?",
            onConfirm: cancel,
            showConfirm:true
        })
    ...
    return (
        ...
        <>
            <div className="flex justify-start w-full text-sm mt-1">
                <div onClick={()=>handleTable("Joined Groups")} className={`cursor-pointer hover:scale-110 flex items-center rounded-t-xl text-center px-2 p-1  ${(table===j)?"bg-blue-400 text-white":"bg-slate-800/50 "}` }>{(table===j)?j:j.split(" ")[0]}</div>
                <div onClick={()=>handleTable("Waiting Approval")} className={`cursor-pointer hover:scale-110 flex items-center rounded-t-xl text-center px-2 p-1 ${(table===w)?"bg-blue-400 text-white":"bg-slate-800/50 "} `}>{(table===w)?w:w.split(" ")[0]}</div>
            </div>
                        ...
                        <tbody className="">
                            {
                                waitingApproval.filter( (g) =>
                                    ...
                                    map((g,i) => (
                                        <tr key={i} className="text-sm odd:bg-blue-400/20 even:bg-gray-300/30">
                                            <td className="p-3">{i+1}</td>
                                            <td className="p-3">{g.groupName}</td>
                                            <td className="p-3">{g.groupCode}</td>
                                            <td className="">
                                                <button 
                                                    onClick={()=>cancelRequest(g.groupCode)}
